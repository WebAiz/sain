...
export function Stuff(props: Props) {
  ...
  return (
    <main className={'stuff'}>
      ...
      <button onClick={() => setAddMode(true)}>Добавить нового сотрудника
      </button>
      {addMode &&
        <Modal onClose={() => setAddMode(!addMode)}>
          <form onSubmit={handleSubmit} className="form border p-10">
            <h2>Добавление нового Сотрудник</h2>
            ...
            <button className={'big-btn'} type="submit">Добавить нового
              сотрудника
            </button>
          </form>
        </Modal>}
      {editMode && <Modal onClose={() => setEditMode(false)}>
        <form onSubmit={saveEdit} className="form border">
          <h2>Изменить данные сотдрудника</h2>
          <div className="form__body">
            <div className="form__field">
              <label htmlFor="address">ФИО</label>
              <input
          ...
          <button className='big-btn' type="submit">Сохранить</button>
        </form>
      </Modal>}
    </main>
  );
